...
public class LevelSeqPrint <Key extends Comparable<Key>, Value>{
	...
	private LevelSeqPrint<Key, Value>.Node min(LevelSeqPrint<Key, Value>.Node x) {
		if (x == null)
			return null;
		if (x.left == null)
			return x;
	...
	private LevelSeqPrint<Key, Value>.Node deleteMin(LevelSeqPrint<Key, Value>.Node x) {
		if (x == null)
			return null;
		if (x.left == null)
			return x.right;
	...
	private LevelSeqPrint<Key, Value>.Node delete(LevelSeqPrint<Key, Value>.Node x, Key key) {
		if (x == null)
			return null;
		int cmp = key.compareTo(x.key);
		if (cmp > 0)
	...
	private LevelSeqPrint<Key, Value>.Node max(LevelSeqPrint<Key, Value>.Node x) {
		if (x == null)
			return null;
		if (x.right == null)
			return x;
	...
	private void keys(LevelSeqPrint<Key, Value>.Node x, Key low, Key high, LinkList<Key> listKeys) {
		if (x == null)
			return;
	...
	private int height(LevelSeqPrint<Key, Value>.Node x) {
		if (x == null)
			return -1;
	...
	void print(LinkList<Node> stack) {
		...
		LinkList<Node> next = new LinkList<LevelSeqPrint<Key, Value>.Node>();
		while (!stack.isEmpty()) {
	...
	public void printLevelNoRecursion() {
		LinkList<Node> outStack = new LinkList<LevelSeqPrint<Key, Value>.Node>();
		outStack.addLast(root);
		
		while(!outStack.isEmpty()) {
			Node x = outStack.removeFirst();
			System.out.print(x.key + "->" + x.val + " ");
			LinkList<Node> innerStack = new LinkList<LevelSeqPrint<Key, Value>.Node>();
			if(x.left!=null) {
				innerStack.addLast(x.left);
			}
			if(x.right!=null) {
				innerStack.addLast(x.right);
			}
			while(!innerStack.isEmpty()) {
				outStack.addLast(innerStack.removeFirst());
			}
		}
	}
	
	public static void main(String[] args) {
		LevelSeqPrint<String, Integer> seqPrint=new LevelSeqPrint<String, Integer>();
		
		for(int i=0;i<10;i++)
		seqPrint.put("s-"+i, i);
		
		seqPrint.printLevelNoRecursion();
	}
